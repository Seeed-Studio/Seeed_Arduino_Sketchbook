...
//  delay(1000);
  ...
  tft.readRectRGB(0, 0, 1, 1, color); // 1 x 1 so reading 1 pixel at 0,0
  ...
  for (uint32_t py = 0; py < (tft.height() - 1); py++)
  {
    for (uint32_t px = 0; px < (tft.width() - 1); px++)
    {
      uint32_t i = px + tft.width() * py;
      yield();
      if (i)
      {
        tft.readRectRGB(px, py, 1, 1, color);
  ...
  sameColorPixelCount = (uint32_t)tft.width() * (uint32_t)tft.height() - sameColorStartIndex;
  if (sameColorPixelCount > 65535)
  {
    sameColorPixelCount16 = 65535;
    printHex16(&sameColorPixelCount16, 1);
